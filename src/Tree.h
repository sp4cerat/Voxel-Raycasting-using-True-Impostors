...
//			int m = mem[(((i*2/3)+(k*y/vz)*x*z+(j*3/4+vy/4)*x)<<shift)+add]; bonsai
	...
	void loadPLY(char* filename, bool octree_mode = false){
		...
		{
			vertex_arr[i]=    (vertex_arr[i]-x_min)/dx; i++;
			vertex_arr[i]=1.0-(vertex_arr[i]-y_min)/dy; i++;
			vertex_arr[i]=    (vertex_arr[i]-z_min)/dz; i++;
		};
		...
		for(int i=0;i<=c;i++)
		{
			...
			for(int j=0;j<=d;j++)
			{
				...
				if(octree_render)
				{
					...
					uchar4 color;
					color.x =int(float(n.x * 127+128))^voxel_x;
